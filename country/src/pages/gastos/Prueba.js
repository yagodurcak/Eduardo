...
const [exito, setExito] = useState(false);
    ...
    const customerTableHead = [
      ...
      // render: data => {for (let i = 0; i <data.water_expenditures.length; i++) {

      //   if (data.water_expenditures[i].date === (data2[data2.length - 1].date)) {
      //     setBase(data.water_expenditures[i].consume)
      //     return parseInt(data.water_expenditures[i].consume) 
      //   } 
  ...
  let costoUnidad = "";
    ...
    const buscarCobranza = async() => {
      ...
      const rtdo = await axios.get(url, {headers})
      const rtdo2 = (rtdo.data.data).filter(artista=> artista.id === 3)
      ...
      const peticionPost2=async()=>{
        ...
        const peticionPost=async(e)=>{
          ...
          f.append("file", selectedImage)
            ...
            const url1= "https://back2.tinpad.com.pe/public/api/import-water-expenditures"
